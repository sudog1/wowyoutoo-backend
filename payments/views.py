...
class CartView(APIView):
    ...
    def post(self, request, product_id):
        ...
        return Response(
            {"message": "Product is added to cart"}, status=status.HTTP_201_CREATED
        )
    ...
    def put(self, request):
        try:
            ...
            # 장바구니에 보여줄 상품 정보 보기 좋게 list 형식으로 자리 바꿈
            transposed_lists = list(
                zip(product_names, descriptions, prices, quantities)
            )
            ...
            # Get the serialized data
            serialized_data = [
                serializer.validated_data for serializer in serialized_data
            ]
...
class CompleteView(APIView):
    ...
    def post(self, request):
        ...
        if access_token:
            ...
            return Response(
                {"error": "access token을 발급받는데 실패했습니다."},
