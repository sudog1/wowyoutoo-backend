...
from .serializers import AnnoucementListSerializer,AnnoncementSerializer,QnaListSerializer,QnaSerializer,QnaResponseSerializer,AdMailSerializer
from .models import Announcement,Qna,QnaResponse
from .permissions import ReadOnlyPermission
from .pagenation import PostPageNumberPagination
from .tasks import send_email,send_email_al
...
class QnaView(APIView):
    ...
    def get(self,request,qna_id=None,format=None):
        if qna_id==None:
            ...
            pagenator = self.pagination_class() #pagenation.py에서 커스터마이징한 페이지네이터 클래스로 인스턴스생성 
            page = pagenator.paginate_queryset(qnas, request)#page의 타입은 리스트입니다,page_size = 15 만큼 원소를 가지며,원소는 qna입니다
            serializer=pagenator.get_paginated_response(QnaListSerializer(page,many=True).data)#제이슨형식으로 반환할 데이터
            return Response(serializer.data,status=status.HTTP_200_OK)
        else:
...
class QnaResponseView(APIView):
    ...
    def get(self,request,qna_id):
        qna=get_object_or_404(Qna.objects.select_related("author"),pk=qna_id)
        if qna.is_private==True and (request.user!=qna.author):#is_private이 true면 자기와 운영자만 볼수있음
